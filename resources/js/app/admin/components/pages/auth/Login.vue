    ...
    <body class="login-page">
        <div class="login-box">
            <div class="login-logo">
                <a><b>{{ $helper.getLang('page.login.title') }}</b></a>
            </div>
            <div class="card">
                <div class="card-body login-card-body">
                    <p class="login-box-msg">{{ $helper.getLang('page.login.slogan') }}</p>
                    <form v-on:submit="login">
                        <div class="error text-danger text-bold text-sm text-center" v-if="formDataError.message">{{formDataError.message}}</div>
                        <div class="input-group mt-3">
                            <input type="email" class="form-control" :placeholder="$helper.getLang('page.user.email')" name="email" v-model="formData.email">
                        ...
                        <div class="input-group mt-3">
                            <input type="password" class="form-control" :placeholder="$helper.getLang('page.user.password')" name="password" v-model="formData.password">
                            <div class="input-group-append">
                                <div class="input-group-text">
                        ...
                        <div class="row text-center mt-3">
                            <div class="col-12">
                                <button type="submit" class="btn btn-primary btn-block" v-if="!isLoginClick">{{ $helper.getLang('button.login') }}</button>
